# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 11:23:16 2024
...
df = pd.read_csv(f'INPUT/{name}/model/kinematic/5000_samples/mean/{name}_mean_kinematic_model.csv')
...
xS_interp = np.arange(patch_interp/2 , 1.0122*ncols*patch,patch_interp)
yS_interp = np.arange(-(nrows*0.96*patch) + patch_interp/2, 0 + 0.8,patch_interp)
...
hlat = df0['lat'].values[nrows-1] + (hlat + offlat/2) * (1/(111)) 
hlon = df0['lon'].values[nrows-1] + (hlon + offlon/2) * (1/(111))
hlon,hlat = -80.22259826328555,0.31008879607685763
hlon,hlat = -80.22883446,0.31329670
...
fig.coast(shorelines = 'thin',borders=["1/1,black"])
with fig.inset(position=f"j{edge}{side}+w{w}c/{h}c+o0.2c/0.2c",box="+p4.0p,black"):
    # Use a plotting function to create a figure inside the inset
    # fig.basemap(
    #     region=[-100, -40, -50,30],
    ...
    rectangle = [[region[0], region[2], region[1], region[3]]]
    fig.plot(data=rectangle, style="r+s", pen="2p,white")
    #fig.savefig('pedernales_ref.pdf')
    fig.savefig('pedernales_ref.png',dpi=1200)
fig.show()
