...
xS_interp = np.arange(patch_interp/2 , 1.005*ncols*patch,0.5*patch_interp)
yS_interp = np.arange(-(nrows*0.95*patch) + patch_interp/2,0.5*patch_interp - 0,patch_interp)
...
#hlon = -71.71518943194556 
#hlat = -31.72624698085415


hlon,hlat = -71.74827760239326,-31.72257287140232

fig.basemap(
    region = region,
    map_scale="jBL+o1c/1c+w50k",
    projection = "M20c",
    frame = ["a0.5f0.5",f"+t Mw8.3 {event}"])
...
fig.colorbar(frame="xa4f2+lSlip (m)",position='jBR+o0.6c/2.2c+w3.2c/0.2c+h',box='+gwhite+p0.8p,black')
...
with fig.inset(position=f"j{edge}{side}+w{w}c/{h}c+o0.2c/0.2c",box="+p4.0p,black"):
    ...
    rectangle = [[region[0], region[2], region[1], region[3]]]
    fig.plot(data=rectangle, style="r+s", pen="2p,white")
    #fig.savefig('illapel_ref.pdf')
    fig.savefig('illapel_ref.png',dpi=1200)
    fig.show()
