...
# for name in ['Tohoku']:
    ...
    labels= [('$V_{r}$','$U/T_{r}$')]
    units = [('(km/s)','(m/s)')]
    for k,pair in enumerate([(Vr,slip_velocity)]):
        ...
        for i in ids_greater:
            ...
            sns.kdeplot(data,x='x',y='y',ax =ax,levels = [1-0.68],linewidths=0.5,color=colors[j],alpha=alphas[j])
        x_mean = np.mean(pair[0],axis=1)
        y_mean = np.mean(pair[1],axis=1)
        # commnet out if corr coef relevant 
        ...
        ax.scatter(x_mean[ids_greater],y_mean[ids_greater], color=colors[j],alpha = alphas[j],s=4.5,label ='$M_w$'+f'{mag} {name}')
        ...
        ax.set_title(f'{labels[k][1]} vs {labels[k][0]} for ' + f'$U >{th_ratio}$'+'$U_{max}$',fontsize=8.75,fontweight='bold')
        ax.set_xlabel(f'{labels[k][0]} {units[k][0]}')
        ax.set_ylabel(f'{labels[k][1]} {units[k][1]}')
        #axes[k][0].set_aspect('equal','box')
        #if k==0 or k==1:
            
        #    axes[k//2][k%2].axhline(y=th_ratio*np.max(U_mean),linestyle=lns[],linewidth = lws[m],color='k')
        #    axes[k//2][k%2].text(lims[name][k],(th_ratio + 0.015)*np.max(U_mean),f'${th_ratio}$'+'$U_{max}$',fontsize=5)
        ax.legend(labelspacing = 0.1, fontsize=7)
        ax.set_aspect(1.5)
        ...
        # if m==0:
        #     if name!= 'Iquique':
        #         fig.colorbar(im,ax=axes[k//2][k%2+2],shrink=0.5,label= 'Correlation',orientation='horizontal')
        #     else:
...
fig.suptitle('Kinematic Models for $N_{samples}$'+f'$={samples}$',y=0.98,fontweight = 'bold')
print(name)
if name!= 'Iquique':
...
folder = os.path.join(os.getcwd(),'global_corr/multi_event')
os.makedirs(folder,exist_ok=True)
file_name = os.path.join(folder,f'Vr_UTr_all_samples_{samples}_.png')
fig.savefig(file_name,dpi=400)
plt.close(fig)
...
# for name in ['Tohoku']:
    ...
    for k,pair in enumerate([(Tr,U)]):
        ...
        for i in ids_greater:
            ...
            sns.kdeplot(data,x='x',y='y',ax =ax,levels = [1-0.68],linewidths=0.3,color=colors[j],alpha=alphas[j])
        x_mean = np.mean(pair[0],axis=1)
        y_mean = np.mean(pair[1],axis=1)
        #commnet out if 
        #corrcoef = np.corrcoef(x_mean[ids_greater],y_mean[ids_greater])
...
folder = os.path.join(os.getcwd(),'global_corr/multi_event')
os.makedirs(folder,exist_ok=True)
file_name = os.path.join(folder,f'Tr_U_all_samples_{samples}_.png')
fig.savefig(file_name,dpi=400)
plt.close(fig)
...
folder = os.path.join(os.getcwd(),'global_corr/multi_event')
os.makedirs(folder,exist_ok=True)
file_name = os.path.join(folder,f'Tr_Vr_all_samples_{samples}_.png')
fig.savefig(file_name,dpi=400)
plt.close(fig)
...
folder = os.path.join(os.getcwd(),'global_corr/multi_event')
os.makedirs(folder,exist_ok=True)
file_name = os.path.join(folder,f'Vr_U_all_samples_{samples}_.png')
fig.savefig(file_name,dpi=400)
plt.close(fig)
