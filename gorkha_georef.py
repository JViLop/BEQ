# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 10:38:08 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 10:27:28 2024
...
patch_interp = patch/4

xS_interp = np.arange(patch_interp/2 , ncols*patch,patch_interp)
yS_interp = np.arange(-(nrows*0.98*patch) + patch_interp/2, 0 + 3,patch_interp)
...
# data from GeoMap
lon_trench = np.array([
    65.028, 64.161, 65.028, 65.492, 65.958, 66.53, 67.104, 67.268, 67.253, 67.248,
    67.332, 67.342, 67.24, 67.277, 67.301, 67.849, 68.311, 68.679, 69.086, 69.881,
    70.1, 70.178, 70.258, 70.101, 69.942, 70.439, 70.942, 71.392, 71.847, 72.459,
    73.03, 73.593, 74.314, 75.089, 75.675, 75.722, 76.521, 77.007, 77.641, 78.076,
    78.422, 78.912, 79.197, 79.79, 80.227, 80.547, 81.043, 81.361, 82.2, 82.935,
    83.667, 84.159, 84.649, 85.622, 86.252, 86.481, 86.82, 87.219, 87.745, 88.32,
    89.282, 89.772, 90.329, 91.258, 91.781, 92.305, 93.233, 93.57, 94.189, 94.829,
    95.466, 96.234, 96.792, 95.969, 96.117, 96.31, 96.581, 96.906, 97.13, 97.346,
    97.471
])

# data from GeoMap
lat_trench = np.array([
    24.382, 24.35, 24.382, 24.62, 24.856, 25.179, 25.499, 25.984, 26.518, 27.085,
    27.912, 28.404, 28.573, 29.377, 29.795, 30.073, 29.878, 29.209, 29.096, 29.262,
    29.84, 30.423, 31.006, 31.459, 31.913, 32.431, 32.947, 33.477, 34.005, 34.682,
    34.176, 33.667, 33.159, 32.881, 32.189, 31.633, 31.154, 30.738, 30.558, 30.226,
    29.887, 29.678, 29.393, 29.143, 29.093, 28.975, 28.79, 28.413, 28.167, 27.882,
    27.593, 27.452, 27.309, 27.17, 27.095, 26.737, 26.668, 27.022, 26.909, 26.885,
    27.027, 26.847, 26.937, 27.007, 27.033, 27.056, 27.117, 27.11, 27.501, 27.918,
    28.183, 28.135, 28.11, 23.76, 24.48, 25.134, 25.647, 26.287, 26.82, 27.241,
    27.702
])
...
hlon,hlat = 84.73582760000176,28.08883405333439
hlon,hlat = 84.7354135,28.08729691
fig.basemap(
    region = region,
    map_scale="jBL+o1c/1c+w50k",
    projection = f"M{size}c",
    frame = ["a0.5f0.5",f"+t Mw7.8 {event}"]
)
#frame = ["a0.5f0.5",f"+t Mw7.8 {event}"]
fig.coast(shorelines = 'thin',borders=["1/1,black"],land="gray",water="lightskyblue",map_scale="jBL+o1c/1c+w50k")
pygmt.makecpt(
           transparency = 80,
           cmap="hot",
           series=[0,12,4],
...
fig.plot(
    x=lon_trench,  # Longitude in degrees East
    y=lat_trench,  # Latitude in degrees North
    # Draw a 2-points thick, red, dashed line for the survey line
    pen="3.5p",
    style="f1c/0.25c+l+t+o0.3c+p",
    fill="black"
)
fig.colorbar(frame="xa4f2+lSlip (m)",position='jBR+o0.6c/2.25c+w3c/0.2c+h',box='+gwhite+p0.8p,black')

fig.plot(x = hlon,y = hlat,style="a1.2c", pen="1p,black", fill="blue")
#fig.plot(x = 85.26,y = 27.77,style="a1.2c", pen="1p,black", fill="black")
...
with fig.inset(position=f"j{edge}{side}+w{w}c/{h}c+o0.2c/0.2c",box="+p4.0p,black"):
    ...
    rectangle = [[region[0], region[2], region[1], region[3]]]
    fig.plot(data=rectangle, style="r+s", pen="2p,white")
    fig.savefig('gorkha_ref.png',dpi=1200)
    #fig.savefig('gorkha_ref.pdf')
    fig.show()
