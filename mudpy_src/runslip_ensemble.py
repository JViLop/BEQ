...
#                makedirs(subfault_folder)
    ...
    if static==1 and okada==True:
        ...
        mpi='mpiexec -n '+str(ncpus)+' python '+mud_source+'parallel_ensemble.py run_parallel_green '+home+' '+project_name+' '+station_file+' '+model_name+' '+str(dt)+' '+str(NFFT)+' '+str(static)+' '+str(dk)+' '+str(pmin)+' '+str(pmax)+' '+str(kmax)+' '+str(tsunami)+' '+str(insar)
        print(mpi)
        mpi=split(mpi)
...
def make_parallel_teleseismics_green(home,project_name,station_file,fault_name,model_name,teleseismic_vel_mod,time_epi,endtime,ncpus,hot_start=0):
    ...
    mpi='mpiexec -n '+str(ncpus)+' python '+mud_source+'parallel_ensemble.py run_parallel_teleseismics_green '+home+' '+project_name+' '+str(time_epi)+' '+station_file+' '+model_name+' '+teleseismic_vel_mod+' '+str(endtime)
    print(mpi)
    mpi=split(mpi)
    p=subprocess.Popen(mpi)
    p.communicate()
...
#Now make synthetics for source/station pairs
def make_parallel_synthetics(home,project_name,gf_project_name,station_file,fault_name,model_name,integrate,static,tsunami,beta,
    ...
    station_file=home+gf_project_name+'/data/station_info/'+station_file
    fault_file=home+project_name+'/data/model_info/'+fault_name
    #Time for log file
    now=datetime.datetime.now()
    now=now.strftime('%b-%d-%H%M')
    ...
    mud_source=environ['MUD']+'/src/python/mudpy/'
    mpi='mpiexec -n '+str(ncpus)+' python '+mud_source+'parallel_ensemble.py run_parallel_synthetics '+home+' '+project_name+' '+gf_project_name+' '+station_file+' '+model_name+' '+str(integrate)+' '+str(static)+' '+str(tsunami)+' '+str(time_epi)+' '+str(beta)+' '+str(custom_stf)+' '+str(impulse)+' '+str(insar)+' '+str(okada)+' '+str(mu)
    print(mpi)
    mpi=split(mpi)
    p=subprocess.Popen(mpi)
    p.communicate()
