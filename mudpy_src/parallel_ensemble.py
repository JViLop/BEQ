...
def run_parallel_synthetics(home,project_name,gf_project_name,station_file,model_name,integrate,static,tsunami,time_epi,
    ...
    #Load structure
    model_file=home+gf_project_name+'/structure/'+model_name
    ...
    for ksource in range(len(mpi_source)):
        ...
        if static==0 and tsunami==0:  #Where to save dynamic waveforms
            green_path = home+gf_project_name+'/GFs/dynamic/'+model_name+"_"+strdepth+".sub"+subfault+"/"
            save_path = home+project_name+'/GFs/dynamic/'+model_name+"_"+strdepth+".sub"+subfault+"/"
        if static==1 and tsunami==1:  #Where to save dynamic waveforms
            green_path=home+gf_project_name+'/GFs/tsunami/'+model_name+"_"+strdepth+".sub"+subfault+"/"
            save_path = home+project_name+'/GFs/tsunami/'+model_name+"_"+strdepth+".sub"+subfault+"/"

        if static==1 and tsunami==0:  #Where to save statics
            green_path=home+gf_project_name+'/GFs/static/'+model_name+"_"+strdepth+".sub"+subfault+"/"
            save_path = home+project_name+'/GFs/static/'+model_name+"_"+strdepth+".sub"+subfault+"/"
        ...
        if write_df==True and static ==1: #Note to self: stop using 0,1 and swithc to True/False
            ...
            SSdf.to_csv(save_path+'subfault'+num+'.SS.static.neu',sep='\t',index=False,header=False)
            ...
            DSdf.to_csv(save_path+'subfault'+num+'.DS.static.neu',sep='\t',index=False,header=False)
...
if __name__ == '__main__':
    ...
    if sys.argv[1]=='run_parallel_green':
        ...
        home=sys.argv[2]
        project_name=sys.argv[3]
        gf_project_name=sys.argv[4]
        station_file=sys.argv[5]
        model_name=sys.argv[6]
        integrate=int(sys.argv[7])
        static=int(sys.argv[8])
        ...
        run_parallel_synthetics(home,project_name,gf_project_name,station_file,model_name,integrate,static,tsunami,time_epi,beta,custom_stf,impulse,rank,size,insar,okada,mu_okada)
