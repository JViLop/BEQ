...
def waveforms_fakequakes(home,project_name,gf_project_name,fault_name,rupture_list,GF_list,
                model_name,run_name,dt,NFFT,G_from_file,G_name,source_time_function='dreger',zeta=0.2,
                stf_falloff_rate=4.0,rupture_name=None,epicenter=None,time_epi=None,Mw=None,
                hot_start=0):
    '''
    ...
    Nss,Ess,Zss,Nds,Eds,Zds=load_fakequakes_synthetics(home,project_name,gf_project_name,fault_name,model_name,GF_list,G_from_file,G_name)
    print('... ... done')
    ...
    for ksource in range(hot_start,len(all_sources)):
        ...
        if rupture_list!=None:
            #Get epicentral time
            epicenter,time_epi=read_fakequakes_hypo_time(home,gf_project_name,rupture_name)
            forward=False
        else:
            forward=True #This controls where we look for the rupture file
        
        # Put in matrix
        m,G=get_fakequakes_G_and_m(Nss,Ess,Zss,Nds,Eds,Zds,home,project_name,gf_project_name,rupture_name,time_epi,GF_list,epicenter,NFFT,source_time_function,stf_falloff_rate,zeta,forward=forward)
        # Solve
        waveforms=G.dot(m)
        #Write output
        write_fakequakes_waveforms(home,project_name,gf_project_name,rupture_name,waveforms,GF_list,NFFT,time_epi,dt,loc_epi = epicenter ,Mw = Mw)
...
def load_fakequakes_synthetics(home,project_name,gf_project_name,fault_name,model_name,GF_list,G_from_file,G_name):
    ''''
...
def get_fakequakes_G_and_m(Nss,Ess,Zss,Nds,Eds,Zds,home,project_name,gf_project_name,rupture_name,time_epi,GF_list,epicenter,NFFT,
...
def write_fakequakes_waveforms(home,project_name,gf_project_name,rupture_name,waveforms,GF_list,NFFT,time_epi,dt,loc_epi = [50,50],Mw=7.0):
    '''
...
#    rupture=rupture_name.split('.')[0]+'.'+rupture_name.split('.')[1]
    ...
    sta=genfromtxt(home+gf_project_name+'/data/station_info/'+GF_list,usecols=0,dtype='U') 
    lon=genfromtxt(home+gf_project_name+'/data/station_info/'+GF_list,usecols=1)  
    lat=genfromtxt(home+gf_project_name+'/data/station_info/'+GF_list,usecols=2) 
    ...
    epicenter,time_epi,Mw=loc_epi, time_epi, Mw 
