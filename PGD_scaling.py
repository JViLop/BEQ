...
for i,name in enumerate(names):
    ...
    plt.errorbar(dr_sorted,peak_ground[ind_dr_sorted]*100, yerr =std_peak_ground[ind_dr_sorted]*100,mec='k',mew=0.2,elinewidth =0.4,capsize=0.75,fmt='.',label='$M_w$'+f'{magnitude} {name}',ms=msizes[i],color=colors[i])
    Mw =[6,7,8,9]
    r = [30,125,320,700]
    for i,m in enumerate(Mw):
        ...
        plt.text(r[i],10**(-4.434 + 1.047*m - 0.138*m*np.log10(r[i])),f'Mw{m}',fontsize=6)
    ...
    plt.ylabel('PGD (cm)')
    plt.legend(loc='lower left',handletextpad=0.02,borderaxespad=0.1,fontsize=8)
    
plt.savefig('PGD_fk_deformation_scaling.png', dpi=1200)
plt.close()
...
for i,name in enumerate(names):
    ...
    plt.errorbar(azimuth_sorted,peak_ground[ind_azimuth_sorted]*100, yerr =std_peak_ground[ind_azimuth_sorted]*100,mec='k',mew=0.2,elinewidth =0.4,capsize=0.75,fmt='.',ms=msizes[i],color=colors[i])

      
    plt.yscale('log')
    plt.ylim(3,1e4)
    plt.xlabel('Angle from along-dip line (°)')
    plt.ylabel('PGD (cm)')
    # plt.legend(loc='lower left',handletextpad=0.02,borderaxespad=0.1,fontsize=8)

    #plt.legend(loc = 'upper left', ncol=3,handletextpad=0.02,borderaxespad=0.1,fontsize=8)
    

plt.savefig('PGD_azimuth_deformation_scaling_from _strikeline.png', dpi=900)
