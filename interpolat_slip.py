...
T0 = np.loadtxt('Rupture_Time_Tohoku_all_kinematic_model.txt')
T0 = T0.reshape(nrows,ncols)
...
UPERP = np.flip(df['U_perp'].values.reshape(nrows,ncols,order='F'),axis=0)
UPARALLEL = np.flip(df['U_parallel'].values.reshape(nrows,ncols,order='F'),axis=0)
VR = np.flip(df['Vr'].values.reshape(nrows,ncols,order='F'),axis=0)
TR = np.flip(df['Tr'].values.reshape(nrows,ncols,order='F'),axis=0)
...
Vr_interp = RegularGridInterpolator((yS, xS), UPARALLEL,bounds_error=False, fill_value=None,method = method)
Tr_interp = RegularGridInterpolator((yS, xS), UPARALLEL,bounds_error=False, fill_value=None,method = method)
t0_interp = RegularGridInterpolator((yS, xS), UPARALLEL,bounds_error=False, fill_value=None,method = method)
...
# im0 = axes[0].pcolormesh(xS_interp,yS_interp,Slip_interp,cmap='YlOrRd')
# fig.colorbar(im0,ax = axes[0],shrink = 0.4,label= 'Slip interp (m)',)
# im1 = axes[1].pcolormesh(xS,yS,Slip,edgecolors='k',cmap='YlOrRd',lw=0.01)
# fig.colorbar(im1,ax = axes[1],shrink = 0.4,label= 'Slip (m)')



xstn,ystn = set_stn(1,2,patch)
...
plt.tight_layout()
main_dir = os.getcwd()
fig_dir  = os.path.join(main_dir,'Tests_interpolation')
