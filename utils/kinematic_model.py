# -*- coding: utf-8 -*-
"""
Created on Wed Nov  1 09:20:24 2023
...
class Kinematic_model():
    directory_folder = os.path.join(os.getcwd(),'kinematic')
    ...
    def resampled_models_(self):
        ...
        try: 
            os.mkdir(Kinematic_model.directory_folder)  
            print(f'Directory {Kinematic_model.directory_folder}  just created')
        except FileExistsError:
            print(f'Directory {Kinematic_model.directory_folder} already created')
        filename  = f"n_{self.nsamples}_sampled_models_{self.type_model}_{self.name}.dat"    
        output_file = os.path.join(Kinematic_model.directory_folder,filename)
    ...
    def mean_(self):
        ...
        mean_d = {
            ...
            'Tr':self.mean[432:648],
            'Vr':self.mean[648:864],
            'Hypo_as': self.mean[864:865], # along-strike
            'Hypo_dd': self.mean[865:], # down-dip
                    }
        self.mean_d = mean_d
    ...
    def std_(self):
        ...
        std_d = {
            ...
            'std_U_parallel':self.std[216:432],
            'std_Tr':self.std[432:648],
            'std_Vr':self.std[648:864],
            'std_Hypo_as': self.std[864:865], # along-strike
            'std_Hypo_dd': self.std[865:] # down-dip
                }
        self.std_d = std_d
    ...
    def skew_(self):
        skew_d = {
            ...
            'skew_U_parallel':self.skewness[216:432],
            'skew_Tr':self.skewness[432:648],
            'skew_Vr':self.skewness[648:864],
            'skew_Hypo_as': self.skewness[864:865], # along-strike
            'skew_Hypo_dd': self.skewness[865:] # down-dip
                }
        self.skew_d = skew_d
    ...
    def cov_m(self):
        ...
        std_cov = {
            ...
            'std_Tr_cov':np.sqrt(np.diag(self.cov_m)[432:648]),
            'std_Vr_cov':np.sqrt(np.diag(self.cov_m)[648:864]),
            'std_Hyp_as_cov':np.sqrt(np.diag(self.cov_m)[864:865]),
            'std_Hyp_dd_cov':np.sqrt(np.diag(self.cov_m)[865:866]),
            }
        # computing std for U  = U(Uper,Uparallel)
        
        cov_pp = self.cov_m[:216,216:432]
    ...
    def full_model_(self):
        ...
        try: 
            os.makedirs(Kinematic_model.path_data)  
            print(f'Directory {Kinematic_model.path_data}  just created')
        except FileExistsError:
            print(f'Directory {Kinematic_model.path_data} already created')
        
        data_path = os.path.join(Kinematic_model.path_data,f'full_mean_{self.type_model}_model_{self.name}.csv')
    ...
    def distribution(self,parameter,color_bar_label,skew=False):
        ...
        ax.invert_yaxis()
        try: 
            os.makedirs(Kinematic_model.path_plots)  
            print(f'Directory {Kinematic_model.path_plots}  just created')
        except FileExistsError:
            print(f'Directory {Kinematic_model.path_plots} already created')
        plot_path = os.path.join(Kinematic_model.path_plots,f'{self.type_model}_{parameter}_{self.name}.jpg')
