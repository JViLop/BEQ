...
if __name__ =='__main__':
    ...
    h5f_name_edks =  f'EDKS_{name}_displacement_nsamples_{nsamples}_parallel.h5'
    h5f_name_okada =  f'Okada_{name}_displacement_nsamples_{nsamples}_parallel.h5'
    ...
    if rank ==0:
        ...
        ranges = [(starts[p], ends[p]) for p in range(nprocs)]
    ...
    for i in range(ranges[0],ranges[1]):   
        print(f'MPI: processor {rank} out of {nprocs} \n Model: {i} in {ranges[0]} to {ranges[1]}')
        Np = nrows*ncols
        data = bin_data[:,i] 
        data0 = np.copy(data)
        if name =='Gorkha':
        ...
        ENZ_ok.append(Displacement.flatten(order='F'))
        ...
        N_ed.append(N_edks)
        Z_ed.append(Z_edks)
        ENZ_ed.append(np.array([E_edks,N_edks,Z_edks]).flatten())
        
    
  
    E_ok = comm.gather(E_ok, root=0)
    N_ok = comm.gather(N_ok, root=0)
    Z_ok = comm.gather(Z_ok, root=0)
    E_ed = comm.gather(E_ed, root=0)
    N_ed = comm.gather(N_ed, root=0)
    Z_ed = comm.gather(Z_ed, root=0)
    ENZ_ok = comm.gather(ENZ_ok,root=0)
    ENZ_ed = comm.gather(ENZ_ed,root=0)

    
   
    if rank == 0:
        # Combine the results from all processes
        E_okada_all = extend_array(E_ok)
        N_okada_all = extend_array(N_ok)
        Z_okada_all = extend_array(Z_ok)
        E_edks_all = extend_array(E_ed)
        N_edks_all = extend_array(N_ed)
        Z_edks_all = extend_array(Z_ed)
        
        ENZ_ok_all = extend_array(ENZ_ok)

        ENZ_ed_all = extend_array(ENZ_ed)
        print(ENZ_ok_all.shape) 
        N_okada,E_okada, Z_okada = np.mean(N_okada_all,axis=0),np.mean(E_okada_all,axis=0),np.mean(Z_okada_all,axis=0)
        N_edks,E_edks, Z_edks = np.mean(N_edks_all,axis=0),np.mean(E_edks_all,axis=0),np.mean(Z_edks_all,axis=0)
  
        
        h5file_edks = h5py.File(h5file_dir_edks,'w')
        h5file_okada = h5py.File(h5file_dir_okada,'w')        
        edks_displacement_dset = h5file_edks.create_dataset('displacement',data = ENZ_ed_all)
        okada_displacement_dset = h5file_okada.create_dataset('displacement',data = ENZ_ok_all)
        h5file_edks.close()
        h5file_okada.close()
        ...
        fig_dir  = os.path.join(working_dir,f'OUTPUT/{name}/model/kinematic/EDKS/parallel/{nsamples}_samples')
