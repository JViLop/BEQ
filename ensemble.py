...
class Ensemble_Displacement(Observable):
    ...
    def bin_data_reader(self):
        ...
        if self.samples == 'all':
            ...
            if self.name == 'Tohoku':
                ...
                ext = 'dat'
            eqs = {'Gorkha':{'kinematic':90000},'Iquique':{'kinematic':72000},'Pedernales':{'kinematic':600000},'Illapel':{'kinematic':393216},'Tohoku':{'kinematic':1000000,'static':2000000}}
            samples = eqs[self.name][self.model_type]
            file_dir = os.path.join(file_folder,f'{self.name}_{self.model_type}_n_{self.samples}.{ext}')
            self.bin_data = np.fromfile(file_dir,'float').reshape((self.nparameters,samples))
        else:
    ...
    def Multiple_Okada_displacement(self):
        ...
        displacement_dset = h5file.create_dataset(f'{self.observable}',shape=(samples,len(self.xstn)*len(self.ystn)*3))
    ...
    def plot_cov(self):
        ...
        # fig, axes = plt.subplots(1,3,figsize=(10,5),dpi=800)
        # for i,parameter_id in enumerate(cov.keys()):
        #     parameter = cov[parameter_id]

        #     title = f"{self.name}  Uncertatinty in {parameter_id} surface {self.observable} "
        ...
        file_folder = self.make_dirs(file='figures_std')
...
class Ensemble_Stress(Observable):
    ...
    def Multiple_Okada_stress(self,stress_drop=True,comp=2):
        self.bin_data_reader()
        self.mean_model_reader()
        self.set_h5file()
        self.source()
        ...
        stress_dset = h5file.create_dataset(f'{self.observable}',shape=(samples,len(self.xstn)*len(self.ystn)*3))
        ...
        for i in range(self.bin_data.shape[1]):
            ...
            for k in range(Np):
                ...
                CS, SS = np.cos(self.strike_rad[k]), np.sin(self.strike_rad[k])
                CD, SD = np.cos(self.dip_rad[k]), np.sin(self.dip_rad[k])
    ...
    def plot_cov(self):
        ...
        #     title = f"{self.name}  Uncertatinty in {parameter_id} {self.observable} "
