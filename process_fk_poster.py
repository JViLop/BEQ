# -*- coding: utf-8 -*-
"""
Created on Fri Nov 22 10:15:49 2024
...
for n,name in enumerate(names):
    ...
    for i,comp in enumerate(['x','y','z']):
        for j in range(nrows):
            ...
            axes[n][i].fill_between(t,lower_lim,upper_lim,alpha=0.5,facecolor=colors[j])    
            axes[n][0].set_title('$M_w%s$'%(magnitude)+f' {name}',fontweight='bold',loc='left',fontsize=16)    
            axes[n][i].set_ylabel(y_labels[i],fontsize=14)
       
            fig.align_ylabels(fig.get_axes())
            plt.subplots_adjust(wspace=0.2,hspace=0.325)
            
fig.supxlabel('Time from mainshock (s)',y=0.0725,fontsize=14)            
fig.savefig(f'all_event_evolution_peak_displacement_{nmodels}.png')
...
nepochs = 5 #20
fig, axes = plt.subplots(2*len(names),nepochs,figsize=(int(nepochs*4),10),dpi=1400)
...
for n,name in enumerate(names):
    ...
    #for k,epoch in enumerate(list(np.linspace(0,75,nepochs))):
        ...
        sigma_Uz = mean_sigma_Z_mesh[:,:,int(epoch)] 

        
        sigma_Uxz = mean_sigma_Z_mesh[:,:,-1] 

        
        cax2 = axes[n+1][k].pcolormesh(x, y,  sigma_Uz,vmin = 0,vmax = np.max(sigma_Uxz),cmap='rainbow')
...
#            ax.add_artist(e)
        ...
        axes[n+1][k].set_aspect('equal','box')
        if k==0:
            axes[n][k].set_title('$M_w%s$'%(magnitude)+f' {name}', loc='left',fontweight='bold',fontsize=15)
            axes[n][k].set_ylabel('Trench-normal distance (km)',fontsize=8.5)
            axes[n][k].set_xlabel('Along-strike distance (km)',fontsize=8.5)
            axes[n][k].tick_params(axis='both',labelsize=8)

            
            #axes[n+1][k].set_title('$M_w%s$'%(magnitude)+f' {name}', loc='left',fontweight='bold',fontsize=8)
            axes[n+1][k].set_ylabel('Trench-normal distance (km)',fontsize=8.5)
            axes[n+1][k].set_xlabel('Along-strike distance (km)',fontsize=8.5)
            axes[n+1][k].tick_params(axis='both',labelsize=8)
        else:
            ...
            axes[n+1][k].set_xticklabels([])
            axes[n+1][k].set_yticklabels([])
            axes[n+1][k].set_xlabel('')
            axes[n+1][k].set_ylabel('')
            axes[n+1][k].get_xaxis().set_ticks([])
            axes[n+1][k].get_yaxis().set_ticks([])
        fig.align_ylabels(fig.get_axes())
        if k==nepochs -1:
            ...
            divider = make_axes_locatable(axes[n+1][k])
            cax = divider.append_axes("right", size="2%", pad=0.075)

            cbar = fig.colorbar(cax2,cax = cax,shrink=shrink,label='$s$ (m)')
            cbar.ax.tick_params(labelsize=8.5) 
        
        fig.subplots_adjust(wspace=0.1,hspace=hspace)
 
fig.savefig('snapshot_xyz_Tohoku_Illapel.png')
