...
def observables(name,parameters,shape,samples,dir_folder= 'histograms',rake=90, RotAngle=None,th=None):
    ...
    data = h5py.File(f'curve_observables/{name}_Stress change_nsamples_{samples}.h5')
    ...
    av_stress_amplitude = []
    av_stress_amplitude_layered = []
    av_potency_amplitude = []
    for i in range(samples):
        ...
        try:
            ...
            av_stress_amplitude_layered.append(stress_drop_amplitude_layered)
            av_potency_amplitude.append(potency_amplitude)
...
EQs = {'Tohoku':{'Mw':9.0,'n':866,'shape':(9,24),'edges_stress':[4,25],'edges_potency':[1,18.5e3/(2*30)],'rake':90,'RotAngle':None,'color':'blue','loc_stress':(15.0,590),'loc_potency':(255,740)},
...
def plot_all_hist_observables(EQs,samples= 1000,thr = [0.1,0.2],folder = 'stress_samples_5000'):
    ...
    for name in list(EQs.keys()):
        ...
        av_stress, av_stress_layered, av_potency =  observables(name,EQs[name]['n'],EQs[name]['shape'],samples,th=0,rake=EQs[name]['rake'],RotAngle=EQs[name]['RotAngle'],dir_folder=folder)
        av10_stress, av10_stress_layered, av10_potency= observables(name,EQs[name]['n'],EQs[name]['shape'],samples,th = thr[0],rake=EQs[name]['rake'],RotAngle=EQs[name]['RotAngle'],dir_folder=folder)
        av20_stress, av20_stress_layered, av20_potency = observables(name,EQs[name]['n'],EQs[name]['shape'],samples,th = thr[1],rake=EQs[name]['rake'],RotAngle=EQs[name]['RotAngle'],dir_folder=folder)
        ...
        axes[1].set_xlim([2e3/(2*30),26e3/(2*30)])
    ...
    plt.savefig(f'{folder}/all_stress_potency_energy_averages_{samples}.pdf')
    # plt.show()


plot_all_hist_observables(EQs,samples = 10000)
